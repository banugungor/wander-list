...
"Wanderlist" (internal repo name `life-list`, bundle id `com.wanderlist.app`) — an Expo/React Native app where users check off World Heritage sites, countries visited, and cuisines tasted. Two more categories (Islands, Capitals) exist as placeholders (`implemented: false`) with no data yet. Turkish is the default UI language; English is the fallback.
...
### The three (of five) implemented categories

`constants/categories.ts` defines `CategoryId = "heritage" | "places" | "cuisine" | "islands" | "capitals"` with per-category `bg`/`fg` colors, icon, and an `implemented` flag. Islands/Capitals are `implemented: false` — UI that lists categories should treat that flag as "show as coming soon, no data source, no visited-toggle wiring yet," not delete the entry.
