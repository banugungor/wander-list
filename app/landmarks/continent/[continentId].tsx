...
export default function LandmarksContinentCountriesScreen() {
  ...
  const countries = useMemo(
    () =>
      (worldData.countries as Country[]).filter(
        (c) => CONTINENT_BY_COUNTRY_ID[c.id] === id && (countByCountry[c.id] ?? 0) > 0,
      ),
    [id, countByCountry],
  );

  const total = countries.reduce((sum, c) => sum + (countByCountry[c.id] ?? 0), 0);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: palette.cream }}>
      ...
      <ProgressCard
        label={t("landmarksPicker.continentLabel", { continent: continentName })}
      ...
      {loading ? (
        ...
        <>
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              const countryTotal = countByCountry[item.id] ?? 0;
              ...
              return (
                <ProgressRow
                  ...
                  fractionLabel={`${countryVisited}/${countryTotal}`}
                  percent={Math.round((countryVisited / countryTotal) * 100)}
                  accentColor={categoryMeta?.fg ?? palette.brand}
                  onPress={() => router.push(`/landmarks/${item.id}`)}
                />
              );
            }}
          />
